...
import com.kzonix.spark.utils.ApplicationFileUtils;
...
public class ApplicationConfigLoader {
  ...
  private static final String CONF_DIRECTORY = "com/kzonix/spark/conf";
  ...
  private static Config withUserValues(final Config config) {
    try {
      final String userConfigFile = defaultString(System.getProperty("user.name")) + ".com.kzonix.spark.conf";
  ...
  private static Config withExternalAdditionalValues(final Config config) {
    ...
    List<File> configFiles = ApplicationFileUtils.findFiles(directory, FILE_SEPARATOR, ".com.kzonix.spark.conf");
